...
- [ ] **Task 4 — FlowNode + HarnessFlow (ReactFlow) (AC: 2)**
  ...
  - [ ] `apps/web/components/flow/harness-flow.tsx` (`'use client'`): `HarnessFlow({ harness, selectedIndex, onSelectBlock })` — monta `nodes/edges` via `harnessToFlow`, registra `nodeTypes`, **read-first** (`nodesDraggable={false}`, `nodesConnectable={false}`, `edgesFocusable={false}`, `elementsSelectable`, `fitView`), `onNodeClick → onSelectBlock(index)`. Marca o nó `selected` quando `index === selectedIndex`. Inclui `<Background/>` e `<Controls showInteractive={false}/>`.
...
- [ ] **Task 6 — Geração e integração no Builder (AC: 1, 2, 3)**
  - [ ] `apps/web/app/(workspace)/builder/actions.ts` (`'use server'`): server action `proposeHarness(description)` → resolve um Provider/Modelo do **env** (`createProviderRegistry` + `ARCHITECT_PROVIDER`/`ARCHITECT_MODEL`) e chama `decompose` de `@robbia/architect`. Retorna o `ArchitectResponse` (union `harness | clarification`) como `Result` serializável. **Segredos só no servidor** (nunca no client). Sem persistência (proposta é efêmera).
  - [ ] `apps/web/components/builder/builder-workspace.tsx` (`'use client'`): eleva o estado do Builder — conversa (turnos), `proposal` (Harness | null), `selectedIndex`, status do agente. Envia a descrição → `proposeHarness` (transição `MascotCore`: `thinking` durante, `done`/`error` depois). Resposta `harness` → popula cards/fluxo; `clarification` → renderiza as perguntas como turno `assistant` na Conversa; `Result` err → estado de erro legível.
...
- Novos: `app/(workspace)/builder/actions.ts`; `components/builder/{block-card,block-list,builder-workspace}.tsx`; `components/flow/{flow-node,harness-flow}.tsx`; `lib/{harness-flow,block-presentation}.ts` (+ testes). Modificados: `app/(workspace)/builder/page.tsx` (1.6), `lib/block-types.ts` (+`blockBorderClass`), `app/design/page.tsx` (usar o helper), `lib/glossary.ts` (+copy), `apps/web/package.json` (+`@xyflow/react`). **Não** alterar outros packages.
